...
function prepareSnapshotTransaction(input,context={}){
  ...
  for(const member of members){
    ...
    if(lifetime){
      ...
      if(Object.values(lifetime.weapons).some((result)=>result.status==='ANOMALY'))reviewReasons.push({source_member_key:member.source_member_key,reason:'weapon_level_anomaly'});
      if(Object.values(lifetime.lifetime_medals).some((result)=>result.status==='ANOMALY'))reviewReasons.push({source_member_key:member.source_member_key,reason:'lifetime_medal_anomaly'});
    }
    if(member.metrics.current_league_clan_medals.status==='ANOMALY')reviewReasons.push({source_member_key:member.source_member_key,reason:'current_league_clan_medal_anomaly'});
    if(member.metrics.membership_episode_clan_medals.status==='ANOMALY')reviewReasons.push({source_member_key:member.source_member_key,reason:'membership_episode_clan_medal_anomaly'});
  }

  for(const member of members){
    for(const [fieldName,provenance] of Object.entries(member.observation.field_provenance||{})){
      if(['CONFLICTING','AMBIGUOUS'].includes(provenance.status)){
        reviewReasons.push({
          source_member_key:member.source_member_key,
          reason:'field_observation_'+provenance.status.toLowerCase(),
